...
function getSuccessMessage(entity: 'cards' | 'participants' | 'invoices', count: number): string {
  if (entity === 'cards') {
    return count === 1 ? t('import.cardSuccess') : t('import.cardsSuccess', { count });
  }
  ...
  return count === 1 ? t('import.invoiceSuccess') : t('import.invoicesSuccess', { count });
}
...
function saveParticipantsToLocalStorage(participants: Participant[]) {
  ...
  localStorage.setItem(
    'invoicemanager:participants',
    JSON.stringify(
      participantsToImport.map((p) => ({
        id: p.id,
        name: p.name,
...
interface PreparedInvoice
  extends Omit<Invoice, 'dueDate' | 'createdAt' | 'updatedAt' | 'transactions'> {
  dueDate: Date;
  createdAt: Date;
  updatedAt: Date;
...
interface PreparedTransaction
  extends Omit<Invoice['transactions'][0], 'date' | 'createdAt' | 'updatedAt'> {
  date: Date;
  createdAt: Date;
  updatedAt: Date;
}
